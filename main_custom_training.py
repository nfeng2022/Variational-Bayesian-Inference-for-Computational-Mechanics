...
if data_generation_flag == 1:
    ...
    y_data_generator = dg.MeasurementData(n_sam=num_data, ne_sam=ne_sam, d_y=y_dim, d_z=z_dim, d_theta=theta_dim, sig_e=sig_e,
                                          sig_eta=sig_eta)
...
num_epoch1 = 200     # number of epoch
num_epoch2 = 200     # number of epoch
...
results_path = f'2d_results_2steps_{num_layers2}layers_{num_epoch2}epoch'
...
# For mean
y = Input(shape=(y_dim,))
logz_mean_inp = Input(shape=(z_dim,))
logz_sig_inp = Input(shape=(z_dim,))
# y = y_data
# initial = tf.keras.initializers.zeros()
theta_mean_hidden_list = [y]
for i in range(num_layers1):
    theta_mean_hidden_temp = Dense(num_neuron, activation='relu')(theta_mean_hidden_list[-1])
    # theta_mean_hidden_bn_temp = BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001)(theta_mean_hidden_temp,
    #                                                                                       training=True)
    theta_mean_hidden_list.append(theta_mean_hidden_temp)
theta_mean = Dense(theta_dim)(theta_mean_hidden_list[-1])


# For variance
theta_sig_hidden_list = [y]
for i in range(num_layers1):
    theta_sig_hidden_temp = Dense(num_neuron, activation='relu')(theta_sig_hidden_list[-1])
    # theta_sig_hidden_bn_temp = BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001)(theta_sig_hidden_temp,
    #                                                                                       training=True)
    theta_sig_hidden_list.append(theta_sig_hidden_temp)
log_theta_sig = Dense(theta_dim)(theta_sig_hidden_list[-1])
theta_sig = tf.math.exp(log_theta_sig)
...
for i in range(num_layers2):
    z_mean_hidden_temp = Dense(num_neuron, activation='relu')(z_mean_hidden_list[-1])
...
z_mean = Dense(z_dim)(z_mean_hidden_list[-1])
# For variance
# initial = tf.keras.initializers.Constant(np.log(0.02))
z_sig_hidden_list = [y]
for i in range(num_layers2):
    z_sig_hidden_temp = Dense(num_neuron, activation='relu')(z_sig_hidden_list[-1])
...
log_z_sig = Dense(z_dim)(z_sig_hidden_list[-1])
...
##########################################################
# Define loss function for training model p(theta|y)
##########################################################
# Compute term in Eq. 17
def term1(log_theta_sig_point):
    return (-0.5*tf.math.reduce_mean(tf.math.reduce_sum(log_theta_sig_point, axis=-1), axis=0)
            - 0.5*theta_dim*math.log(2.*math.pi)-0.5*theta_dim)


# term1_loss = term1(log_theta_sig)
...
def term2(args):
    y_point, theta_mean_point, theta_sig_point = args
    e_data_point = e_data
    theta_std = tf.math.sqrt(theta_sig_point)
    theta_std = tf.expand_dims(theta_std, axis=1)
    theta_mean_point = tf.expand_dims(theta_mean_point, axis=1)
    y_point = tf.expand_dims(y_point, axis=1)
    # Compute theta samples using broadcast rules
    theta_data = e_data_point * theta_std + theta_mean_point
    theta_data = tf.reshape(theta_data, (-1, theta_data.shape[-1]))
    # f_data, _ = dg.MeasurementData.fem_fh_fun_loop_rev(tf.convert_to_tensor(theta_data))
    f_data, _ = TFMapFnLayer()(theta_data)
    l1 = -0.5 * y_dim * math.log(2. * math.pi * sig_e)
    l2 = -0.5 / sig_e * tf.math.reduce_sum((y_point - f_data)**2, axis=-1)
    # Compute the scaled f mapping data
    return l1 + tf.math.reduce_mean(l2)


'''start = time.time()
term2_loss = term2([y, theta_mean, theta_sig])
end = time.time()
print(f'Analysis takes {end-start} seconds.')'''


# Compute the term in Eq. 22
def term3(args):
    theta_mean_point, theta_sig_point = args
    return -0.5 * theta_dim * math.log(2. * math.pi) - 0.5 * tf.reduce_mean(
        tf.math.reduce_sum(theta_sig_point + theta_mean_point ** 2, axis=-1), axis=0)


# Loss function for step 1: Train model predictions for model parameters
def vi_pred_loss_step1(inp, y_pred):
    term1_loss = term1(y_pred[2])
    term2_loss = term2([inp, y_pred[0], y_pred[1]])
    term3_loss = term3([y_pred[0], y_pred[1]])
    return term1_loss-term2_loss-term3_loss


##########################################################
# Train model for approximating p(theta|y)
##########################################################
...
train_step_fn_step1 = tf.function(train_step)
hist_step1 = {'train_loss': np.zeros((num_epoch1, ))}
loss_value = 0.
for epoch in range(num_epoch1):
    ...
    for step, x_batch_train in enumerate(train_dataset_step1):
        loss_value = train_step_fn_step1(vi_pred_model_step1, optimizer, vi_pred_loss_step1, x_batch_train)
    ...
    if (epoch+1) % model_save_freq1 == 0 and flg_print == 1:
        vi_pred_model_step1.save(filepath=results_path+f'/step1/{epoch:02d}-{loss_value:.8f}.h5')

    # Apply learning rate decay strategy
    if epoch % lr_patience == 0 and epoch > 0 and flg_lr_decay == 1:
        if hist_step1['train_loss'][epoch] - hist_step1['train_loss'][epoch-lr_patience] > 0:
    ...
    hist_step1['train_loss'][epoch] = float(loss_value)

# vi_pred_model_step1.save(filepath=results_path+'/step1/final_model_step1.h5')
h5.write(data=hist_step1, filename=results_path+'/step1/train_hist_step1.h5')
...
vi_pred_model_step2 = Model([y, logz_mean_inp, logz_sig_inp], [theta_mean, theta_sig, z_mean, z_sig, log_theta_sig, log_z_sig])

# Compute sample estimates of z from q(theta|y)
vi_pred_model_step1_simp = Model(y, [theta_mean, theta_sig])
...
_, h_sam = fem_fh_fun_loop_rev_tf(tf.constant(theta_sam, dtype=tf.float64))
...
vi_pred_test = Model(y, [theta_mean, theta_sig, z_mean, z_sig, log_theta_sig, log_z_sig])
